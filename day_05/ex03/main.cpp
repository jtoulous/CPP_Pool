...
int main()
{
  try 
  {
...

    {
      ...
      ptr = slave.makeForm("ERROR", "bob");
      delete ptr;
      std::cout << "\n" << std::endl;

      ptr = slave.makeForm("Presidential pardon", "bob");
      highRank.signForm(*ptr);
      highRank.executeForm(*ptr);
      delete ptr;
      std::cout << "\n" << std::endl;

      ptr = slave.makeForm("Robotomy request", "bob");
      highRank.signForm(*ptr);
      highRank.executeForm(*ptr);
      delete ptr;
      std::cout << "\n" << std::endl;

      ptr = slave.makeForm("Shrubbery creation", "bob");
      highRank.signForm(*ptr);
      highRank.executeForm(*ptr);
      delete ptr;
